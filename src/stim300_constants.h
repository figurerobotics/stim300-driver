...
static constexpr uint16_t MAX_SERVICEMODE_REPLY_SIZE = 2000;
...
constexpr std::array<bool, 5> isIncluded(DatagramIdentifier datagram_identifier) {
  ...
  return {true, false, false, false, false};  // Should never get here so return RATE settings
}

static inline DatagramIdentifier toDatagramID(std::array<bool, 5> isIncluded) {
  for (size_t i = 0; i < datagram_info_map.size(); ++i) {
    if (datagram_info_map[i].included_sensors == isIncluded) {
      return datagram_info_map[i].id;
    }
  }
  return DatagramIdentifier::RATE_AUX;  // This is to suppress a warning as we never hit here anyway
}

static inline uint8_t calculateDatagramSize(DatagramIdentifier datagram_identifier) {
  if (datagram_identifier == DatagramIdentifier::CONFIGURATION ||
